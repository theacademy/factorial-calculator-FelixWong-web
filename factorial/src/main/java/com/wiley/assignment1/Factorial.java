...
public class Factorial {
	...
	public int isInputNumberValid() {
		...
		// Attempted witout try catch, but it is need if user puts an invalid input such
		// as abc
		try {
			...
			// we dont need to specify specif error types, any errors should return the same
			// statement
		} catch (Exception e) {
			this.printStream.print("Invalid entry. Please enter an integer between 1 and 10, inclusive.");
			return -1;
	...
	private void calculateFactorial(int num) {
		...
		int result = 1;
		// YOUR CODE STARTS HERE
		// loop down until i = 1 and multiply but its own value along the way
		for (int i = num; i >= 1; i--) {
			result *= i;
		}
		// YOUR CODE ENDS HERE
